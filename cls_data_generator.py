...
# Data generator for allied task data combination
class DataGenerator(object):
    def __init__(self, params, split=1, shuffle=True, per_file=False, is_eval=False):
        ...
        # Batch Sampling
        self._batch_sampling = params.get('batch_sampling', 'OBS')
        self._rare_classes = [12, 2, 10, 7, 3, 11]

        if not self._is_eval and self._batch_sampling == 'BBS':
            self._rare_samples_idx = []
            for idx, filename in enumerate(self._filenames_list):
                label_path = os.path.join(self._label_dir, filename)
                labels = np.load(label_path)  # [frames, nb_class]
                # Multi-Label Case
                #if np.any(labels[:, self._rare_classes]):
                if np.any(labels[..., self._rare_classes]):
                    self._rare_samples_idx.append(idx)
            self._normal_samples_idx = list(set(range(len(self._filenames_list))) - set(self._rare_samples_idx))
    ...
    def generate(self):
        ...
        if self._is_eval:
            ...
            for i in range(self._nb_total_batches):
                ...
                if self._batch_sampling == 'BBS':
                    # Every time batch generate file index of num as self._batch_size, partial from sparse class and partial from normal class
                    rare_ratio = 0.05  # Adjustable
                    num_rare = int(self._batch_size * rare_ratio)
                    num_normal = self._batch_size - num_rare

                    # Avoid rare Samples to be 0
                    if len(self._rare_samples_idx) == 0:
                        rare_idxs = []
                    else:
                        rare_idxs = random.choices(self._rare_samples_idx, k=num_rare)

                    # Avoid normal_idxs too less and report error
                    if num_normal > len(self._normal_samples_idx):
                        normal_idxs = random.choices(self._normal_samples_idx, k=num_normal)
                    else:
                        normal_idxs = random.sample(self._normal_samples_idx, k=num_normal)
                    batch_idxs = rare_idxs + normal_idxs
                    random.shuffle(batch_idxs)
                    file_indices = batch_idxs

                    self._circ_buf_feat = deque()
                    self._circ_buf_label = deque()

                    for idx in file_indices:
                        if idx >= len(self._filenames_list):
                            break
                        temp_feat = np.load(os.path.join(self._feat_dir, self._filenames_list[idx]))
                        temp_label = np.load(os.path.join(self._label_dir, self._filenames_list[idx]))
                    ...
                    while (len(self._circ_buf_feat) < self._feature_batch_seq_len or (
                            hasattr(self, '_circ_buf_vid_feat') and hasattr(self, '_vid_feature_batch_seq_len') and len(
                            self._circ_buf_vid_feat) < self._vid_feature_batch_seq_len)):
                        temp_feat = np.load(os.path.join(self._feat_dir, self._filenames_list[file_cnt]))
                        ...
                        if not self._per_file:
                            ...
                            # remove all frames in features and labels matrix that are outside
                ...
                if self._multi_accdoa is True:
                    ...
                    sed_label = label[:, :, :self._nb_classes]  # (114, 50, 13)
                    doa_label = label[:, :, self._nb_classes:self._nb_classes * 4]  # (114, 50, 13)
                    label = np.concatenate([sed_label, doa_label], axis=-1)  # ⇒ (114, 50, 26)
                yield feat, label
