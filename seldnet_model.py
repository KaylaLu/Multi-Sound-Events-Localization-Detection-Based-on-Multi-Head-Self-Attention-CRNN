#
# The MHSA-CRNN model architecture
#
...
from parameters import Class_Distribution, LossType

# Class-sample number-based reweighting
def get_class_weights(class_distribution, smooting=0.01):
    distribution = np.array(class_distribution, dtype=np.float32)
    weights = 1. / (distribution + smooting)
    weights = weights / np.sum(weights) * len(distribution)
...
class MSELoss_ADPIT(object):
    ...
    def __init__(self,gamma = 2):
        ...
        self.class_weights = get_class_weights(Class_Distribution)
        self.gamma = gamma  # focal loss Parameter

    def _focal_mse_loss(self, output, target):
        # output, target: [batch, frames, tracks*axes, classes]
        diff = output - target
        focal_weight = torch.pow(1 - torch.exp(-torch.abs(diff)), self.gamma)
        loss = focal_weight * (diff ** 2)
        return loss

    def _each_calc(self, output, target):
        if LossType == 'focal':
            # output, target: [batch, frames, tracks*axes, classes]
            loss = self._focal_mse_loss(output, target)  # [batch, frames, tracks*axes, classes]
            weights = self.class_weights.to(loss.device)  # [classes]
            loss = loss * weights  # Reweighting
            return loss.mean(dim=2)  # mean over track*axis, shape=[batch, frames, classes]
        else:
