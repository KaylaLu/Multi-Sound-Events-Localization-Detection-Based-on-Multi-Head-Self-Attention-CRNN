...
class FeatureClass:
    ...
    def extract_all_feature(self):
        ...
        wav_list = sorted(glob(os.path.join(self._aud_dir, '**', '*.wav'), recursive=True))

        for file_cnt, wav_path in enumerate(wav_list):
            base_name = os.path.basename(wav_path).split('.')[0]
            feat_path = os.path.join(self._feat_dir, '{}.npy'.format(base_name))
            try:
                self.extract_file_feature((file_cnt, wav_path, feat_path))
            except Exception as e:
                print(f"[ERROR] Skipping {wav_path}: {e}")

        print(f"Finished in {time.time() - start_s:.2f} seconds")
    ...
    def extract_all_labels(self):
        ...
        for sub_folder in os.listdir(self._desc_dir):
            ...
            for file_cnt, file_name in enumerate(os.listdir(loc_desc_folder)):
                '''Kayla, July7th, feature extraction'''
                if not file_name.strip() or file_name.startswith('.'):
                    continue
                if file_name.startswith("._") or not file_name.endswith(".csv"):
                    continue
                wav_filename = '{}.wav'.format(file_name.split('.')[0])
