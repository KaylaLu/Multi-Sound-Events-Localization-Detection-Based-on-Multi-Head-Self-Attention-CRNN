...
class ComputeSELDResults(object):
    ...
    # Collect Prediction Results as file
    ...
    def get_consolidated_SELD_results(self, pred_files_path, score_type_list=['all', 'room']):
        ...
        for score_type in score_type_list:
            print('\n\n------------------------------------  {}   ---------------------------------------------'.format('Total score' if score_type == 'all' else 'score per {}'.format(score_type)))
...
# Reshape vector 3D to 2D
def reshape_3Dto2D(A):
    return A.reshape(A.shape[0] * A.shape[1], A.shape[2])

# Main script part
if __name__ == "__main__":
    pred_output_format_files = 'Submissions/Task_A'  # Path of the DCASEoutput format files
