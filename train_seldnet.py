...
#
print("Starting train_seldnet.py", flush=True)
...
print('SELD model imported, start training', flush=True)

# Split ACCDOA presentation data labels, improved to include distance information
def get_accdoa_labels(accdoa_in, nb_classes):
    x, y, z = accdoa_in[:, :, :nb_classes], accdoa_in[:, :, nb_classes:2*nb_classes], accdoa_in[:, :, 2 * nb_classes:3 * nb_classes]
    dist = accdoa_in[:, :, 3 * nb_classes:4 * nb_classes]
    dist[dist < 0.] = 0.
    sed = np.sqrt(x ** 2 + y ** 2 + z ** 2) > 0.5
    doa = accdoa_in[:, :, :3 * nb_classes]
    return sed, doa, dist

# Split Multi-ACCDOA presentation data labels
...
# Location similarity detection by threshold
...
# Eval Mode Epoch Pipeline, using pretrained models
...
def test_epoch(data_generator, model, criterion, dcase_output_folder, params, device):
    print("Using Test Epoch Module...", flush=True)
    # Number of frames for a _60 second audio with 100ms hop length = 600 frames
    ...
    with torch.no_grad():
        for values in data_generator.generate():
            ...
            output = model(data)
            '''if is_first_batch:
                print(f"[DEBUG] test_data.shape = {data.shape}")
                print(f"[DEBUG] test_target.shape = {target.shape}")
                print(f"[DEBUG] test_output.shape = {output.shape}")
            ...
            try:
                ...
                print("[DEBUG] train_output.shape:", output.shape)
                print("[DEBUG] train_target.shape:", target.shape)
                print("[DEBUG] train_output sample:", output[0, 0, :10])
                print("[DEBUG] train_target sample:", target[0, 0, :10])
                raise e'''
                ...
                for frame_cnt in range(sed_pred.shape[0]):
                    for class_cnt in range(sed_pred.shape[1]):
                        if sed_pred[frame_cnt][class_cnt]>0.5:
                            ...
                            output_dict[frame_cnt].append([class_cnt, doa_pred[frame_cnt][class_cnt],
                                                           doa_pred[frame_cnt][class_cnt + params['unique_classes']],
                                                           doa_pred[frame_cnt][class_cnt + 2*params['unique_classes']], dist_pred[frame_cnt][class_cnt]])
            data_generator.write_output_format_file(output_file, output_dict)

            test_loss += loss.item()
            nb_test_batches += 1
            if params['quick_test'] and nb_test_batches == 3:
                break
...
def train_epoch(data_generator, optimizer, model, criterion, params, device):
    ...
    for values in data_generator.generate():
        ...
        nb_train_batches += 1
        if params['quick_test'] and nb_train_batches == 3:
            break
...
def print_param_size(model, title="Model"):
    ...
    try:
        ...
        elem_size, dtype = 4, "N/A"
    mem_mb = trainable * elem_size / (1024**2)
    print(f"{title} | trainable params: {trainable:,} (+ buffers: {buffers:,}; total: {total:,})")
    print(f"≈ Parameter memory: {mem_mb:.2f} MB (dtype={dtype})")
...
def main(argv):
    ...
    # Test code to detect if CUDA available
    print("CUDA Available:", torch.cuda.is_available())
    print("Using device:", device)
    print("CUDA Device Name:", torch.cuda.get_device_name(device) if torch.cuda.is_available() else "CPU")
    ...
    if params['mode'] == 'dev':
        ...
        val_splits = [[4]]
        train_splits = [[3]]
        print("Go for split module", flush = True)
        ...
        for split_cnt, split in enumerate(test_splits):
            ...
            # -------- CSV LOGGING --------
            # format:[epoch_cnt, train_loss, F, AE, Dist_err, Rel_dist_err, SELD]
            csv_Name = "Mode999Print"
            if parameters.compMode == 'mac':
                csv_save_path = f"/Volumes/WDBlack1T/PlotData/TableCSVs/{csv_Name}.csv"
            else:
                csv_save_path = f"D:/FProj/PlotData/TableCSVs/{csv_Name}.csv"
            ...
            print_param_size(model, title=f"SELDnet | dataset={params['dataset']} | multi_accdoa={params['multi_accdoa']}")
            ...
            for epoch_cnt in range(nb_epoch):
                ...
                # Generate epoch-wise loss information, stored as .csv files
                csv_writer.writerow([
                    int(epoch_cnt),
                    f"{train_loss:.6f}",
                    f"{val_loss:.6f}",
    ...
    if params['mode'] == 'eval':
        ...
        print("Load best model weights", flush=True)
        model_name = os.path.join(params['model_dir'], params['pretrained_model_weights'])
        if not os.path.exists(model_name):
            raise FileNotFoundError(f"Pretrained weights not found: {model_name}")
